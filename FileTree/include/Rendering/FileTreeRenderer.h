...
public:
    ...
    ~FileTreeRenderer() {}
    enum class CallbackType { Click, DoubleClick, ContextMenu };

private:
    std::shared_ptr<FileTree> m_FileTree;
    std::unique_ptr<IFileDialogManager> m_fileDialog;
    struct OpenFile {
        std::string content;
        std::string path;
    } m_CurrentOpenFile;
...
private:
    ...
    void TriggerExtensionCallback(const std::string& extension,
                                  CallbackType type,
...
public:
    ...
    void RegisterExtensionCallback(const std::string& extension,
                                   CallbackType type,
