...
std::unique_ptr<FileNode> FileTree::buildFileTree(const fs::path& _folder) {
    auto filename =
        _folder.filename().empty() ? fs::path(_folder).wstring() : _folder.filename().wstring();
        ...
        for (const auto& entry : fs::directory_iterator(_folder, ec)) {
            ...
            try {
                ...
                if (fs::is_directory(entry, ec)) {
                    auto childNode =
                        std::make_unique<FileNode>(filePath.filename().wstring(), FileType::DIR);
                    ...
                    auto fileNode =
                        std::make_unique<FileNode>(filePath.filename().wstring(), FileType::FILE);
