...
FileTreeRenderer::FileTreeRenderer(const std::shared_ptr<FileTree>& _fileTree)
    : m_FileTree{_fileTree}, m_fileDialog{IFileDialogManager::Create()} {}
...
void FileTreeRenderer::RenderOpenFile() {
    ...
    ImGui::Begin(m_CurrentOpenFile.path.c_str(),
                 nullptr,
...
void FileTreeRenderer::RenderFileTreeContextMenu(FileNode* _node) {
    if (ImGui::BeginPopupContextItem()) {
        ...
        if (_node->type == FileType::FILE && ImGui::MenuItem("Open File")) {
            fs::path fullPath = m_FileTree->getRootNode()->fullPath / _node->name;
            std::string pathStr = fullPath.string();
            m_CurrentOpenFile.content = ImGuiUtils::readFile(fullPath); // TODO: Replace this
        ...
        if (_node->type == FileType::FILE) {
            ...
            std::transform(
                loweredExt.begin(), loweredExt.end(), loweredExt.begin(), [](unsigned char c) {
            ...
            if (m_callbackRegistry.count(loweredExt) > 0 &&
                ...
                if (ImGui::MenuItem(menuLabel.c_str())) {
                    m_callbackRegistry[loweredExt][CallbackType::ContextMenu](
                        _node->fullPath.string());
                }
            }
        }
...
void FileTreeRenderer::HandleDoubleClickNode(FileNode* _node) {
    if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
        ...
        if (_node->type == FileType::FILE) {
            m_CurrentOpenFile.content = ImGuiUtils::readFile(fullPath);
            m_CurrentOpenFile.path = pathStr;
...
void FileTreeRenderer::RegisterExtensionCallback(const std::string& extension,
                                                 CallbackType type,
...
void FileTreeRenderer::TriggerDirectoryCallback(CallbackType type,
...
void FileTreeRenderer::TriggerExtensionCallback(const std::string& extension,
                                                CallbackType type,
                                                const std::filesystem::path& path) {
    std::string ext = extension;
    std::transform(
